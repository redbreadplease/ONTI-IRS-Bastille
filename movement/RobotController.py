...
class RobotController:
    ...
    def do_front_align(self):
        ...
        err = round((max(min(abs(d1 - d2) * self.P_koef, 255), 30)), 1)
        if d1 - d2 > self.min_react_value:
            self.is_align_finished = False
            self.move_clockwise(err)
        elif self.min_cliff_value > d2 - d1 > self.min_react_value:
            self.is_align_finished = False
            self.move_counterclockwise(err)
        else:
            mid = (d1 + d2) / 2.
            if mid > self.right_distance + self.wall_dist_deviation:
                self.is_align_finished = False
                self.move_straight(mid - self.right_distance)
            elif mid < self.right_distance - self.wall_dist_deviation:
                self.is_align_finished = False
    ...
    def do_back_align(self):
        ...
        err = round((max(min(abs(d1 - d2) * self.P_koef, 255), 30)), 1)
        if self.min_cliff_value > d1 - d2 > self.min_react_value:
            self.is_align_finished = False
            self.move_counterclockwise(err)
        elif self.min_cliff_value > d2 - d1 > self.min_react_value:
            self.is_align_finished = False
